...
async def signup(req: SignupRequest):
    """회원가입"""
    data = await auth_service.signup(req)
...
async def login(req: LoginRequest, request: Request):
    """로그인"""
    data = await auth_service.login(req, request)
...
async def logout(request: Request):
    """로그아웃"""
    data = await auth_service.logout(request)
    return StandardResponse.success(SuccessCode.SUCCESS, data)
...
async def get_me(user: Dict = Depends(get_current_user)):
    """내 정보 조회 (로그인 상태 검증)"""
    data = await auth_service.getMe(user)
    return StandardResponse.success(SuccessCode.SUCCESS, data)
...
async def check_email_availability(email: str = Query(..., description="중복 확인할 이메일")):
    """이메일 중복 체크"""
    data = await auth_service.checkEmailAvailability(email)
    return StandardResponse.success(SuccessCode.SUCCESS, data)
...
async def check_nickname_availability(nickname: str = Query(..., description="중복 확인할 닉네임")):
    """닉네임 중복 체크"""
    data = await auth_service.checkNicknameAvailability(nickname)
    return StandardResponse.success(SuccessCode.SUCCESS, data)
