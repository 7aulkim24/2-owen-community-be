...
# --- Configuration ---
# Replace this with your actual Backend EC2 Public IP or Domain
BACKEND_API_URL="http://YOUR_BACKEND_IP:8000"

# Frontend Repository URL
REPO_URL="https://github.com/7aulkim24/2-owen-community-fe.git"
REPO_DIR="/home/ubuntu/frontend"
...
# --- Patch Code for Dynamic Configuration ---
echo "Patching frontend code for dynamic configuration..."

# 1. Inject <script src="/config.js"></script> into all HTML files in pages/
# This ensures window.APP_CONFIG is available before other scripts run.
find pages -name "*.html" -exec sed -i 's|</head>|<script src="/config.js"></script></head>|' {} +

# 2. Overwrite js/api.js to use window.APP_CONFIG
cat <<EOF > js/api.js
import { getErrorMessage, getSuccessMessage } from './error-messages.js';

// API 베이스 URL 설정
// window.APP_CONFIG.API_BASE_URL (Runtime Config) 우선, 없으면 localhost
...
# Create config.js with the actual backend URL
echo "window.APP_CONFIG = { API_BASE_URL: '$BACKEND_API_URL' };" > /var/www/html/config.js

# Configure Nginx for extensionless URL routing
...
server {
    ...
    location / {
        # Handle extensionless URLs by trying .html
